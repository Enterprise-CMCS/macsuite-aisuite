...
DocxSource = Union[Path, BinaryIO]
...
def extract_pdf_pages(pdf_source) -> List[Dict[str, Any]]:
    repeated_lines = collect_repeated_lines(pdf_source)
...
def parse_pdf_to_records(file_bytes: bytes, doc_id: str) -> List[Dict[str, Any]]:
    pdf_source = BytesIO(file_bytes)
    pages = extract_pdf_pages(pdf_source)
    return build_section_json(pages, doc_id)

def parse_docx_to_records(file_bytes: bytes, doc_id: str) -> List[Dict[str, Any]]:
    doc = Document(file_bytes)
    records = []

 # put page number as page 1, page 2, etc. since docx does not have printed page numbers
