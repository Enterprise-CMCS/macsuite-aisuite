...
RESULT_COLUMNS = ["AI Recommendation", "AI Reasoning", "Citation Source"]
...
class ExcelRAGProcessor:
    ...
    def _remove_excel_protection(self):
        if self.workbook.security:
            self.workbook.security.lockStructure = False
            self.workbook.security.lockWindows = False
    ...
    def _detect_header_row_and_requirement_col(self):
        for row_num in range(1, MAX_HEADER_SCAN_ROWS + 1):
            for col_num in range(1, self.worksheet.max_column + 1):
                ...
                if value is not None and str(value).strip() == REQUIREMENT_COL:
                    return row_num, col_num
 
        raise ValueError(
            f"Could not find '{REQUIREMENT_COL}' within first {MAX_HEADER_SCAN_ROWS} rows "
            f"of sheet '{self.sheet_name}'"
        )
    ...
    def _ensure_result_columns(self):
        ...
        for col_name in RESULT_COLUMNS:
            ...
            self.worksheet.column_dimensions[new_letter].width = self.worksheet.column_dimensions[prev_letter].width
    ...
    async def process_requirement(self, requirement: str, retry_unclear: bool = True) -> dict:
        try:
            ...
            if recommendation == "NOT MET" and retry_unclear:
                retry_prompt = (
                    "Based on the available documentation, please determine if the following requirement is MET or NOT MET. "
                    "If there is insufficient information, explain what specific information is missing.\n\n"
                ...
                retry_text = retry_response.output if hasattr(retry_response, "output") else str(retry_response)
    ...
    def _parse_agent_response(self, response_text: str) -> dict:
        try:
            ...
            source_with_page = self._limit_citation_sources(source, max_sources=5, max_pages_per_source=5)
    ...
    async def process_all(
        ...
        for row_num in range(self.header_row + 1, self.worksheet.max_row + 1):
            requirement = self.worksheet.cell(row=row_num, column=self.requirement_col).value
        ...
        for i, row_num in enumerate(rows_to_process, start=1):
            requirement = str(
                self.worksheet.cell(row=row_num, column=self.requirement_col).value
            ).strip()
            ...
            if output_path and save_every > 0 and self.processed_count % save_every == 0:
...
async def main():
    data_dir = Path(r"c:\Moktari\code\rag_results\state_of_HI\MCCRS-HI-6775")
    original_file = data_dir / "HI CCS SFY25-26 SC6 Rate Tool-_6775_1.xlsm"
